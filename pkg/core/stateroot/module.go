...
// AdvanceForkIndex records a new block index while keeping the current state root.
// Used by nanvil fork mode where MPT nodes are not available locally.
func (s *Module) AdvanceForkIndex(index uint32, cache *storage.MemCachedStore) (*mpt.Trie, *state.MPTRoot) {
	root, _ := s.currentLocal.Load().(util.Uint256)
	sr := &state.MPTRoot{Index: index, Root: root}
	s.addLocalStateRoot(cache, sr)
	s.currentLocal.Store(sr.Root)
	s.localHeight.Store(sr.Index)
	return s.mpt, sr
