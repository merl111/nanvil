...
// Export returns copies of all key-value pairs in the store.
func (s *MemoryStore) Export() (mem, stor map[string][]byte) {
	s.mut.RLock()
	defer s.mut.RUnlock()
	...
	stor = make(map[string][]byte, len(s.stor))
	for k, v := range s.stor {
		stor[k] = append([]byte(nil), v...)
	}
	return mem, stor
}

// Import replaces store contents with the provided key-value pairs.
func (s *MemoryStore) Import(mem, stor map[string][]byte) {
	s.mut.Lock()
	defer s.mut.Unlock()
	if s.mem == nil {
