...
func (bc *Blockchain) storeBlock(block *block.Block, txpool *mempool.Pool) error {
	...
	if bc.config.NanvilForkMode {
		mptTrie, sr = bc.stateRoot.AdvanceForkIndex(block.Index, cache.Store)
	} else {
		var mptErr error
		mptTrie, sr, mptErr = bc.stateRoot.AddMPTBatch(block.Index, b, cache.Store)
		if mptErr != nil {
			// Release goroutines, don't care about errors, we already have one.
			<-aerdone
			...
			return fmt.Errorf("error while trying to apply MPT changes: %w", mptErr)
		}
	}
	...
	if mptTrie != nil && persistedHeight == block.Index-1 {
	...
	mptTrie.Store = bc.dao.Store
	bc.stateRoot.UpdateCurrentLocal(mptTrie, sr)
	bc.topBlock.Store(block)
...
// Flush persists pending in-memory state to the backing store without stopping the chain.
...
func (bc *Blockchain) InitVerificationContext(ic *interop.Context, hash util.Uint160, witness *transaction.Witness) error {
	if len(witness.VerificationScript) != 0 {
		if witness.ScriptHash() != hash {
			got := witness.ScriptHash()
			return fmt.Errorf("%w: expected %s (%s), got %s (%s)",
				ErrWitnessHashMismatch,
				hash.StringLE(), address.Uint160ToString(hash),
				got.StringLE(), address.Uint160ToString(got))
		}
...
func (bc *Blockchain) verifyTxWitnesses(t *transaction.Transaction, block *block.Block, isPartialTx bool, verificationFee ...int64) error {
	...
	for i := range t.Signers {
		if impersonate.Global().Enabled() && impersonate.Global().IsImpersonated(t.Signers[i].Account) {
			continue
		}
