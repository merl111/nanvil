...
// UnknownTransactionError returns a C# RpcServer-compatible unknown transaction
// error with the hash in both message and data fields.
func UnknownTransactionError(hash string) *Error {
	h := hash
	if len(h) >= 2 && h[0:2] != "0x" && h[0:2] != "0X" {
		h = "0x" + h
	}
	return NewError(ErrUnknownTransactionCode, "Unknown transaction - "+h, h)
}
