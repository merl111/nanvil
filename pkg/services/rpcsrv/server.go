...
func (s *Server) handleInternal(req *neorpc.Request, sub *subscriber) (*neorpc.Response, error) {
	...
	handler, ok := lookupHandler(req.Method)
	if ok {
...
func (s *Server) calculateNetworkFee(reqParams params.Params) (any, *neorpc.Error) {
	...
	for i, signer := range tx.Signers {
		...
		// Wallets (e.g. NeoLine) submit placeholder witnesses before signing.
		if w.ScriptHash() != signer.Account {
			if script, ok := accounts.LookupVerificationScript(signer.Account); ok {
				w.VerificationScript = script
			}
		}
		isNanvil := s.chain.GetConfig().Magic == netmode.NanvilNet
		if isNanvil && w.ScriptHash() != signer.Account &&
			(scparser.IsSignatureContract(w.VerificationScript) || scparser.IsMultiSigContract(w.VerificationScript)) {
			gasConsumed, witnessSize := fee.Calculate(s.chain.GetBaseExecFee(), w.VerificationScript)
		...
		var witnessSize int
		if err != nil {
			if isNanvil && errors.Is(err, core.ErrInvalidSignature) &&
				(scparser.IsSignatureContract(w.VerificationScript) || scparser.IsMultiSigContract(w.VerificationScript)) {
				gasConsumed, witnessSize = fee.Calculate(s.chain.GetBaseExecFee(), w.VerificationScript)
			} else if errors.Is(err, core.ErrInvalidSignature) {
				gasConsumed = 0
				witnessSize = io.GetVarSize(w.VerificationScript) + io.GetVarSize(w.InvocationScript)
			} else {
...
func (s *Server) getTokenTransfers(ps params.Params, isNEP11 bool) (any, *neorpc.Error) {
	...
	// Nanvil uses genesis-era block timestamps during early blocks; wallets pass
...
func (s *Server) getrawtransaction(reqParams params.Params) (any, *neorpc.Error) {
	...
	if err != nil {
		// NeoLine treats JSON-RPC errors as fatal when hydrating transfer history
		// and hangs if any cached tx hash from a prior dev session is missing.
		if s.chain.GetConfig().Magic == netmode.NanvilNet {
			return json.RawMessage("null"), nil
		}
		return nil, s.unknownTransactionError(txHash)
	}
	if v, _ := reqParams.Value(1).GetBoolean(); v {
...
func (s *Server) getTransactionHeight(ps params.Params) (any, *neorpc.Error) {
	...
	if err != nil || height == math.MaxUint32 {
		...
		return nil, s.unknownTransactionError(h)
	}
...
func (s *Server) runScriptInVM(t trigger.Type, script []byte, contractScriptHash util.Uint160, tx *transaction.Transaction, b *block.Block, nextH *uint32, verbose bool) (*result.Invoke, *neorpc.Error) {
	...
	if err != nil {
		...
		s.log.Warn("script execution faulted",
			zap.String("trigger", t.String()),
			zap.String("fault", faultException))
	}
...
func (s *Server) unknownTransactionError(hash util.Uint256) *neorpc.Error {
	if unknownTxResolver != nil {
		return unknownTxResolver(hash)
	}
	return neorpc.UnknownTransactionError(hash.StringLE())
}

// RelayErrorToRPC maps a transaction relay error to a JSON-RPC error.
func RelayErrorToRPC(err error, hash util.Uint256) *neorpc.Error {
	_, rerr := getRelayResult(err, hash)
	return rerr
}
...
func (s *Server) sendrawtransaction(reqParams params.Params) (any, *neorpc.Error) {
	...
	relayErr := relay(tx)
	if relayOutcomeCallback != nil {
		relayOutcomeCallback(relayErr, tx.Hash())
	}
	return getRelayResult(relayErr, tx.Hash())
}
...
func (s *Server) logRequestError(r *params.Request, jsonErr *neorpc.Error) {
	...
	case neorpc.ErrUnknownTransactionCode:
		// Wallets poll pending tx hashes until confirmed; unknown hashes are routine.
		if r.In != nil && r.In.Method == "getrawtransaction" &&
			s.chain.GetConfig().Magic == netmode.NanvilNet {
			s.log.Debug(logText, logFields...)
		} else {
