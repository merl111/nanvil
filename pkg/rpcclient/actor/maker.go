...
// transaction relative to the current blockchain height. When the node reports
// MaxValidUntilBlockIncrement, that is used ("height + increment"). Otherwise
// the shorter "height + number of validators + 1" formula is used.
func (a *Actor) CalculateValidUntilBlock() (uint32, error) {
	...
	if inc := a.version.Protocol.MaxValidUntilBlockIncrement; inc > 0 {
		// GetBlockCount() is one more than the current block height.
		return blockCount - 1 + inc, nil
	}
