...
type (
	...
	Server struct {
		...
		onTransactionRelayed  func(*transaction.Transaction)
...
func (s *Server) relayTxn(t *transaction.Transaction, direct bool) error {
	...
	if err == nil {
		if s.onTransactionRelayed != nil {
			s.onTransactionRelayed(t)
		}
		if direct {
...
// SetOnTransactionRelayed sets a callback invoked after a transaction is
// successfully added to the mempool via RelayTxn/RelayTxnDirectly.
func (s *Server) SetOnTransactionRelayed(fn func(*transaction.Transaction)) {
	s.onTransactionRelayed = fn
}
